...
func (s *SessionReplay) loadSubSet(queryAPI api.QueryAPI, lapID string) DataSubSet {
	...
	for result.Next() {
		...
		currentLapTime := fmt.Sprintf(FormatString[CurrentLapTime], GetFloatValue(values["lapCurrentLapTime"], 0))
		deltaToBestLap := fmt.Sprintf(FormatString[Delta], GetFloatValue(values["lapDeltaToBestLap"], 0))
